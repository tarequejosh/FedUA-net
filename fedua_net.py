...
def build_loaders(df, batch_size, workers):
    loaders = {}
    for split in ('train', 'val', 'test'):
        ...
        loaders[split] = DataLoader(
            ...
            num_workers=workers, pin_memory=True, drop_last=(split == 'train'),
...
class SharedBody(nn.Module):
    """Feature extractor: backbone features + CBAM + GAP + PReLU + MC dropout."""
    def __init__(self, backbone='efficientnet_v2_s', emb=512, dropout=0.3):
        super().__init__()
        ...
        self.cbam = CBAM(cin, ratio=cfg.CBAM_RATIO, kernel=cfg.CBAM_KERNEL)
...
class ClientNet(nn.Module):
    ...
    def __init__(self, num_classes, backbone='efficientnet_v2_s', emb=512, dropout=0.3):
        super().__init__()
        self.body = SharedBody(backbone, emb, dropout)
...
def main():
    ...
    print('  Aggregation weights:', {k: round(v, 4) for k, v in agg_w.items()})

    loaders = {cid: build_loaders(client_dfs[cid], cfg.BATCH_SIZE, cfg.WORKERS)
               for cid in range(cfg.NUM_CLIENTS)}
    cweights = {cid: class_weights(client_dfs[cid]) for cid in range(cfg.NUM_CLIENTS)}
    ...
    for rnd in range(1, cfg.COMM_ROUNDS + 1):
        ...
        log_rows.append(row)
        print(f'  R{rnd:>3}  LRb={lr_backbone:.1e}  ' +
              '  '.join(f'C{c}={client_evals[c]["acc"]:.3f}' for c in sorted(client_evals)) +
              f'  mean={mean_acc:.3f}  ({time.time()-t0:.1f}s)')
