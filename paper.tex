...
The shared visual backbone (EfficientNetV2-S with CBAM) contains $\approx 21.45\text{M}$ parameters. In each communication round, each active hospital node transmits $\approx 85.8\text{ MB}$ of payload (in standard 32-bit floating point precision) or $\approx 42.9\text{ MB}$ under 16-bit half precision. At the edge, inference latency on an NVIDIA RTX GPU is $\approx 8.4\text{ ms}$ per image ($\approx 42\text{ ms}$ on a standard multi-core CPU). Post-hoc temperature scaling and APS set generation require $<0.15\text{ ms}$ per sample, imposing negligible overhead on edge clinical deployment.
...
\subsection{Multi-Task Diagnostic Performance}
Table~\ref{tab:main_results} and Fig.~\ref{fig:main_benchmark} present quantitative diagnostic metrics over 3 random seeds. Under uniform server aggregation, FedUA-Net achieves a multi-task mean accuracy of \textbf{$93.30 \pm 1.13\%$}, macro F1-score of \textbf{$93.05 \pm 1.36\%$}, and MCC of \textbf{$0.894 \pm 0.019$}. 

Crucially, FedUA-Net numerically outperforms all evaluated standard federated baselines, with the performance advantage reaching statistical significance against FedBN and FedProx:
\begin{itemize}
    \item Numerically outperforms FedAvg ($92.36 \pm 0.92\%$) by \textbf{$+0.94\%$} ($p=0.1639$, paired Student's $t$-test).
    \item Statistically significantly outperforms FedBN ($92.34 \pm 0.99\%$) by \textbf{$+0.96\%$} ($p=0.0467$, $\alpha=0.05$).
    \item Numerically outperforms FedBABU ($91.99 \pm 0.35\%$) by \textbf{$+1.31\%$} ($p=0.1091$).
    \item Statistically significantly outperforms FedProx ($91.97 \pm 1.03\%$) by \textbf{$+1.33\%$} ($p=0.0019$, $\alpha=0.01$).
\end{itemize}
...
\begin{figure}[!t]
\centering
\includegraphics[width=0.48\textwidth]{paper_figures/fig7_cka_similarity_heatmap.png}
\caption{Cross-modality layer-wise Linear Centered Kernel Alignment (CKA) similarity matrix across participating clinical clients. Early convolutional stages exhibit high domain invariance ($\text{CKA} = 0.885$), whereas deep convolutional stages, CBAM attention ($\text{CKA} = 0.694$), and latent bottleneck projections ($\text{CKA} = 0.446$) undergo task-specific divergence to accommodate acoustic vs. photonic imaging physics.}
\label{fig:cka}
\end{figure}
...
Furthermore, Fig.~\ref{fig:cka} presents the layer-wise Linear Centered Kernel Alignment (CKA) similarity matrix across client pairs. The representational alignment dynamics demonstrate that:
\begin{enumerate}
    \item \textbf{Early Invariant Primitives:} Stage 1 early convolutional filters exhibit very high cross-modality similarity ($\text{CKA} = 0.885$), confirming that fundamental visual primitives (edge detectors, boundary curvatures, and gradient transitions) are universally transferable across medical imaging modalities.
    \item \textbf{Intermediate Hierarchy:} Stages 3 and 5 maintain substantial representational alignment ($\text{CKA} = 0.724$ and $0.861$), capturing structural tissue contours.
    \item \textbf{Deep Modality Specialization:} In deeper layers, CBAM attention ($\text{CKA} = 0.694$) and the final $512$-D latent projection ($\text{CKA} = 0.446$) progressively specialize. Decoupling local classification heads and Batch Normalization statistics enables these deeper layers to absorb modality-specific physical phenomena (Rayleigh speckle in ultrasound vs. tissue density in radiography) without destabilizing the shared global representation.
\end{enumerate}
