"""Reconstruct per-round fed curves from train_v4.log text and plot the figure."""
import re, os, sys
import pandas as pd
import matplotlib
matplotlib.use('Agg')
...
log = r'D:/Research/FedUA-Net/outputs_v4/train_v4.log'
out = r'D:/Research/FedUA-Net/outputs_v4'
rows = []
...
df.to_csv(os.path.join(out, 'reports', 'v4_fed_log.csv'), index=False)
...
ax.set_title('FedUA-Net v4 - federated learning phase', fontweight='bold')
ax.legend(fontsize=8); ax.set_xticks(df['round']) if len(df) <= 15 else None
ax.set_ylim(0.5, 1.0)

ax = axes[1]
fin = pd.read_csv(os.path.join(out, 'reports', 'v4_final_client_summary.csv'))
bar_names = ['v1\nglobal', 'MobileNetV2\ncentralized', 'v4\npersonalized FL']
